...
impl EnergyBudget {
    ...
    pub fn allocate_proportional(&self, current: &EnsembleEnergy) -> EnsembleEnergy {
        ...
        let agents = current
            ...
            .map(|a| {
                ...
                RhythmicEnergy::new(a.agent_id, new_total * ratio, new_total * (1.0 - ratio))
            })
            .collect();
        EnsembleEnergy::new(agents)
    }
    ...
    pub fn reserve(&self, amount: f64) -> EnergyBudget {
        EnergyBudget::new((self.total_budget - amount).max(0.0), self.strategy.clone())
    }
...
mod tests {
    ...
    fn test_proportional_allocation() {
        ...
        // Both agents had equal total energy (10 each), so each receives half
        // the new budget. Their kinetic/harmonic ratios are preserved.
        assert!((allocated.agents[0].kinetic - 3.0).abs() < 1e-10);
        assert!((allocated.agents[0].harmonic - 7.0).abs() < 1e-10);
        assert!((allocated.agents[1].kinetic - 7.0).abs() < 1e-10);
        assert!((allocated.agents[1].harmonic - 3.0).abs() < 1e-10);
    }
    ...
    fn test_within_budget() {
        let budget = EnergyBudget::new(10.0, AllocationStrategy::Equal);
        let ensemble = EnsembleEnergy::new(vec![RhythmicEnergy::new(0, 3.0, 3.0)]);
